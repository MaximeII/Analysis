...
for j in range(0,len(loc)):
    ...
    if loc[j] == 'mid':
        ...
        loc_cid = loc[j]
        coords = [(xmin+xmax)/4.0,(ymin+ymax)/2.0,(zmin+zmax)/2.0]

    elif loc[j] == 'right':

        loc_cid = loc[j]
        coords = [3.0*(xmin+xmax)/4.0,(ymin+ymax)/2.0,(zmin+zmax)/2.0]
        
    elif loc[j] == 'top_mid':

        loc_cid = loc[j]
        coords = [(xmin+xmax)/2.0,3.0*(ymin+ymax)/4.0,(zmin+zmax)/2.0]
    
    elif loc[j] == 'bot_mid':

        loc_cid = loc[j]
        coords = [(xmin+xmax)/2.0,(ymin+ymax)/4.0,(zmin+zmax)/2.0]

    elif loc[j] == 'top_l':

        loc_cid = loc[j]
        coords = [(xmin+xmax)/4.0,3.0*(ymin+ymax)/4.0,(zmin+zmax)/2.0]

    elif loc[j] == 'bot_l':

        loc_cid = loc[j]
        coords = [(xmin+xmax)/4.0,(ymin+ymax)/4.0,(zmin+zmax)/2.0]

    elif loc[j] == 'top_r':

        loc_cid = loc[j]
        coords = [3.0*(xmin+xmax)/4.0,3.0*(ymin+ymax)/4.0,(zmin+zmax)/2.0]

    elif loc[j] == 'bot_r':

        loc_cid = loc[j]
        coords = [3.0*(xmin+xmax)/4.0,(ymin+ymax)/4.0,(zmin+zmax)/2.0]
    ...
    np.save(path_save+'CellID_'+loc_cid+'.npy',cid)
    print('Saved '+path_save+'CellID_'+loc_cid+'.npy')
