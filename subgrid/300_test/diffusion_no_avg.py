...
for i in range(0,len(loc)):
    ...
    CellID = np.load(path_save+'CellID_'+loc[i]+'.npy')
    ...
    def get_fmu(step):
        ...
        pt.calculations.pitch_angles_box(vlsvReader=pt.vlsvfile.VlsvReader(path_bulk+'bulk.'+str(step).rjust(7,'0')+'.vlsv'),
                                             ...
                                             cosine = True,
                                             outputfile = path_save+'PA/mu'+str(step).rjust(7,'0')+'_'+str(i))
    ...
    np.save(path_save+'dfdmu_'+str(i)+'.npy',dfdmu)
    np.save(path_save+'dfdmumu_'+str(i)+'.npy',dfdmumu)
    ...
    def get_dfdt(step):
        ...
        if step < (bulkStart+int(dt*2.0)):
            ...
            filePA_prev = open(path_save+'PA/mu'+str(step).rjust(7,'0')+'_'+str(i),'r')
            ...
            filePA_next = open(path_save+'PA/mu'+str(step+int(dt*2.0)).rjust(7,'0')+'_'+str(i),'r')
            ...
            filePA_prev = open(path_save+'PA/mu'+str(step-int(dt*2.0)).rjust(7,'0')+'_'+str(i),'r')
            ...
            filePA_next = open(path_save+'PA/mu'+str(step).rjust(7,'0')+'_'+str(i),'r')
    ...
    np.save(path_save+'dfdt_'+str(i)+'.npy',dfdt)
    
    # Get dmu
    lines = []
    
    filePA = open(path_save+'PA/mu'+str(bulkStart).rjust(7,'0')+'_'+str(i),'r')
    ...
    np.save(path_save+'Dmumu_'+str(i)+'.npy',Dmumu)
    print('Saved '+path_save+'Dmumu_'+str(i)+'.npy')
