...
wavemode = '1' #1: use kperp, kpar; =2: use k, thetaa

k     = np.linspace(0.01,1.5,151) # parallel
kperp = k[0]
...
if run == 'BCG' or run == 'BGA':
    ...
    HYDROSfile = open(path_file+run+'_krange_beta2', 'w')
    for line in template:
        if line[:10] == "wavevector":
            HYDROSfile.write('wavevector_mode='+wavemode+'\n')
        elif line[:6] == "kperp=":
            HYDROSfile.write('kperp= '+str(round(kperp,2))+'\n')
        elif line[:5] == "kpar=":
            HYDROSfile.write('kpar= '+str(round(k[0],2))+'\n')
        elif line[:5] == "beta=":
            ...
            HYDROSfile.write('start= [0.05 , -0.]\n')
        elif line[:14] == "normalization=":
            HYDROSfile.write('normalization=1\n')
        elif line[:8] == "scanvar=":
            HYDROSfile.write("scanvar= 'kpar'\n") 
        elif line[:11] == "scan_range=":
            HYDROSfile.write('scan_range= (0.05,'+str(round(k[-1],2))+')\n')
        elif line[:5] == "nval=":
            HYDROSfile.write('nval= '+str(len(k))+'\n')
        elif line[:11] == "resultfile=":
            HYDROSfile.write("resultfile= '"+path_output+run+"_krange_beta2.log'\n")
        else:
            HYDROSfile.write(line)
    HYDROSfile.close() 
    print("Wrote "+run+"_krange_beta2")
    ...
    for i in range(0,len(k)):
        ...
        template   = open(path_HYDROS+'1dscan', 'r')
        HYDROSfile = open(path_file+run+'_beta2_'+str(round(k[i],2)), 'w')
        for line in template:
            if line[:10] == "wavevector":
                HYDROSfile.write('wavevector_mode='+wavemode+'\n')
            elif line[:6] == "kperp=":
                ...
                HYDROSfile.write('kpar= '+str(round(k[i],2))+'\n')
            elif line[:5] == "beta=":
                HYDROSfile.write('beta= '+str(round(beta_para,2))+'\n')
            elif line[:4] == "tau=":
                HYDROSfile.write('tau= '+str(round(tau,2))+'\n')
                ...
                HYDROSfile.write('start= ['+str(w[i])+' , -0.]\n')
            elif line[:14] == "normalization=":
                HYDROSfile.write('normalization=1\n')
            elif line[:8] == "scanvar=":
                HYDROSfile.write("scanvar= 'kpar'\n") 
                ...
                HYDROSfile.write("resultfile= '"+path_output+run+"_beta2_"+str(round(k[i],2))+".log'\n")         
            else:
                HYDROSfile.write(line)
        HYDROSfile.close() 
        print('Wrote '+run+'_beta2_'+str(round(k[i],2))) 
